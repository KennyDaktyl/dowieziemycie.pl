...
/** Homepage fleet preview: one large lead photo plus a strip of
 * thumbnails, every tile opening the same full-screen lightbox as the
 * /flota gallery. Keeps the framed panel look of the fleet section, just
 * makes the photos clickable. */
export function FleetPreviewGallery({ images, name }: { images: GalleryImage[]; name: string }) {
  ...
  return (
    <div className="rounded-[14px] border border-line bg-panel p-3">
      <button
        ...
        className="group block h-[260px] w-full overflow-hidden rounded-[10px] bg-panel-2"
      >
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
          src={absoluteImageUrl(images[0].image)}
          alt={images[0].caption || name}
        ...
        <div className="mt-3 grid grid-cols-4 gap-2">
          {thumbs.map((photo, index) => (
            <button
              ...
              aria-label={t("openPhoto")}
              className="group aspect-[4/3] overflow-hidden rounded-[7px] bg-panel-2"
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
