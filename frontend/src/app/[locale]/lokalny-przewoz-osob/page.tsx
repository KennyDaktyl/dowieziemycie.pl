...
export default async function LokalnyPrzewozPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);

  const [t, tCrumbs, appLocale, page, contact] = await Promise.all([
    getTranslations("LokalnyPrzewoz"),
  ...
  const breadcrumbItems = [{ label: tCrumbs("home"), href: "/" }, { label: tCrumbs("lokalnyPrzewoz") }];
