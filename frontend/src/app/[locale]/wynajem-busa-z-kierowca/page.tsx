...
export default async function WynajemBusaPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);

  const [t, tCrumbs, appLocale, page, contact] = await Promise.all([
    getTranslations("WynajemBusa"),
  ...
  const breadcrumbItems = [{ label: tCrumbs("home"), href: "/" }, { label: tCrumbs("wynajemBusa") }];
