...
import type { ContactInfo, ContentPage } from "@/lib/types";

const SLUG = "nocny-transfer-krakow";
...
export async function generateMetadata({
  ...
  return { title, description, alternates: buildAlternates(`/${SLUG}`, appLocale) };
}

export default async function NocnyTransferPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);

  const [t, tCrumbs, appLocale, page, contact] = await Promise.all([
    getTranslations("NocnyTransfer"),
  ...
  const breadcrumbItems = [{ label: tCrumbs("home"), href: "/" }, { label: tCrumbs("nocnyTransfer") }];
